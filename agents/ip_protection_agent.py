...
class IPProtectionAgent(BaseAgent):
    def __init__(self, retriever):
        super().__init__("IP Protection Agent", retriever)
        # Prompt keeps the agent focused on IP analysis and chunk citations
        self.prompt_template = ChatPromptTemplate.from_messages([
            ("system",
                """
                You are the Intellectual Property (IP) domain expert.
                Evaluate IP obligations and risks using ONLY the provided law chunks.
                Each chunk is labelled with a law name and chunk number.
                Only reference laws from this list: {laws_list}.
                Do not make final compliance decisions—simply report IP implications.
                """
            ),
            ("human",
                """
                ...
                Your Task:
                1. Identify any IP obligations or risks evident in these excerpts (e.g., copyright infringement or exceptions,
                   licensing/clearances for third-party content, terms for user-generated content licenses, notice-and-takedown /
                   safe-harbor requirements such as repeat-infringer policies, trademark use and confusion/brand misuse,
                   counterfeit/unauthorized use, moral/database rights, content identification obligations).
                2. For each cited point, include the jurisdiction and law name (from {laws_list}) and the chunk number(s).
                3. If no IP obligations are found, state 'None'.

                Respond in this format:
                - IP-Protection Concern: [Yes/No/Unclear]
    ...
    def _parse_llm_output(self, text: str) -> dict:
        ...
        try:
            for line in lines:
                if line.startswith("- IP-Protection Concern:"):
                    value = line.replace("- IP-Protection Concern:", "").strip()
